...
from src.camera_profile import CAMERA_PROFILE  # noqa: E402

def parse_args() -> argparse.Namespace: # 경로 설정
    p = argparse.ArgumentParser(description="카메라 동작 확인")
    p.add_argument(
        "--config", type=Path,
        default=ROOT / "config" / f"config_{CAMERA_PROFILE}.yaml",
        help="config.yaml 경로 (기본값은 src/camera_profile.py의 CAMERA_PROFILE 따름)",
    )
    p.add_argument(
